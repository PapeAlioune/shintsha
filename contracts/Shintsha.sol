...
contract Shintsha is ERC721 {
    ...
    */
    ...
    uint256 public loanpaymentPenalty; //@dev in %
    ...
    function registerFarmer(string memory farmname, string memory farmAddress, string memory country, string memory phoneNumber) public returns(bool) {
        ...
        RegisteredFarmers[msg.sender].earnings = 0;
    ...
    function buyProduct(address productOwner, uint256 productId, uint256 contractCut, bool isfiat) public returns(bool) {
        require(msg.sender != address(0), "Invalid sender address");
        ...
        require(productId >= 0, "Product Id must be equal to and greater than 0");
        require(contractCut > 0, "Contract cut has to be greater than 0 Eth");
        totalEarned = totalEarned.add(contractCut);
        RegisteredFarmers[productOwner].registeredProducts[productId].canBeTraded = false;
        if (RegisteredFarmers[msg.sender].active) {
            RegisteredFarmers[productOwner].registeredProducts[productId].earnings = RegisteredFarmers[productOwner].registeredProducts[productId].earnings.add(RegisteredFarmers[productOwner].registeredProducts[productId].sellingPrice.sub(contractCut));
    ...
    function getProduct(uint256 productId) public view returns(string memory name, uint256 sellingPrice, string memory category, uint256 dueDate, address productowner, uint256 cut) {
        require(msg.sender != address(0), "Invalid sender address");
    ...
    function getFarmProductInvestorKeys(uint256 productId) public view returns(address[] memory) {
        require(msg.sender != address(0), "Invalid sender address");
        ...
        return RegisteredFarmers[msg.sender].registeredProducts[productId].investors;
    }

    function getFarmInvestors(address investor) public view returns(string memory name, string memory surname, string memory homeAddress, string memory country, string memory phoneNumber) {
        require(msg.sender != address(0), "Invalid sender address");
        require(RegisteredFarmers[msg.sender].active, "You are currently not registered or are not a farmer");
        require(RegisteredInvestors[investor].active, "Investor does not exist");
        name = RegisteredInvestors[msg.sender].name;
        surname = RegisteredInvestors[msg.sender].surname;
        homeAddress = RegisteredInvestors[msg.sender].homeAddress;
        phoneNumber = RegisteredInvestors[msg.sender].phoneNumber;
        country = RegisteredInvestors[msg.sender].country;
    }
    ...
    function payLoan(uint256 amount) public returns(uint256) {
        require(msg.sender != address(0), "Invalid sender address");
        require(RegisteredFarmers[msg.sender].active, "You currently not registered");
        ...
        if (!ApprovedLoans[msg.sender].paid) {
            if (ApprovedLoans[msg.sender].dateDue < now) {
                if (ApprovedLoans[msg.sender].owing < amount) {
                    ApprovedLoans[msg.sender].owing = 0;
                } else {
                    ApprovedLoans[msg.sender].owing = ApprovedLoans[msg.sender].owing.sub(amount);
                }
